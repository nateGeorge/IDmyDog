...
def do_analysis(breed):
    ...
    pk.dump(histNtext, open('../pickle_files/fgHara-full-13x4-new-new-new.pd.pk', 'wb'))
...
pbar = progressbar.ProgressBar(maxval=1495, widgets=widgets).start() # think maxval should be aronudd bb.shape[0]*6.3, but know its 1495 post-mortem
...
breedsLeft = breeds[245:]

dataSoFar = pk.load(open('../pickle_files/fgHara-full-13x4.pd.pk', 'rb'))

lastIdxs = dataSoFar[dataSoFar.breed=='Whippet'].index.tolist()
print('lastIdxs:', lastIdxs)

rowCnt = lastIdxs[0]
print('rowCnt:', rowCnt)

for breed in breedsLeft:
    cropDir = mainImPath + breed + '/cropped/bodies/'
    fgDir = cropDir + 'fg/'
    fgFiles = os.listdir(fgDir)
